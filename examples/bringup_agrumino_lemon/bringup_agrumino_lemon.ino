...
void setup() {
  ...
  else
  {
    ...
    Serial.println("-----------------------> You need to initilize the memory!");
  }

  /*Serial.println("sizeof(SensorData_t)=" + String(sizeof(SensorData_t)));
    Serial.println("sizeof(SensorDataVector_t)=" + String(sizeof(SensorDataVector_t)));
  ...
  if (Serial.available() > 0) // Serial parser to speed up operations
  {
    ...
    switch (inByte)
    {
      case 'i':
        ...
        Serial.println("\nATTENTION: you need to do the commit if you want to save your data from temporary RAM to FLASH");
        Serial.println();
        break;
      case 'a':
      ...
      case 'd':
        ...
        for (uint16_t i = 0; i < SECTOR_SIZE; i++) {
          Serial.println("EEPROM read addr=" + String(i) + " val=" + String(EEPROM.read(i)));
          Serial.flush();
        }
        break;
      case 's':
