/*
  AgruminoThingSpeakHttpPost.ino - Sample project for Agrumino board using the Agrumino library.
...
void loop() {
  ...
  if (currentIndex == N_SAMPLES - 1) {
    ...
    for (uint8_t i = 0; i < N_SAMPLES; i++) {
      ...
      fieldData[ TEMPERATURA ] = String(PtrFlashMemory->Fields.data.vector[i].temp);
      fieldData[ LUMINOSITA ] = String(PtrFlashMemory->Fields.data.vector[currentIndex].lux);
      fieldData[ TENSIONE ] = String(PtrFlashMemory->Fields.data.vector[currentIndex].batt);
      fieldData[ LIVELLOBATT ] = String(PtrFlashMemory->Fields.data.vector[currentIndex].battLevel);
